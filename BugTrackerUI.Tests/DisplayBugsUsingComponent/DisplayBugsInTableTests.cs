...
namespace BugTrackerUI.Tests.DisplayBugsUsingComponent
{
    public class M606DisplayBugsInTableTests
    {
        ...
        public void DisplayBugsInTableTest()
        {
            ...
            var pattern =
                @"<\s*?table\s*?>[\s\S]*?@foreach\s*?[(]\s*?(var|Bug)\s*bug\s*in\s*Bugs\s*?[)]\s*?{\s*?<\s*?[tT][rR]\s*?>\s*?<[tT][dD]>\s*?@bug.Id\s*?<\s*?\/\s*?[tT][dD]\s*?>\s*?\s*?<[tT][dD]>\s*?@bug.Title\s*?<\s*?\/\s*?[tT][dD]\s*?>\s*?\s*?<[tT][dD]>\s*?@bug.Description\s*?<\s*?\/\s*?[tT][dD]\s*?>\s*?\s*?<[tT][dD]>\s*?@bug.Priority\s*?<\s*?\/\s*?[tT][dD]\s*?>\s*?<\/\s*?[tT][rR]\s*?>\s*?}\s*?<\s*?\/table\s*?>";
            var rgx = new Regex(pattern);
            Assert.True(rgx.IsMatch(file),
                "`BugList.razor` was found, but does not appear to contain a `table` with a `foreach` loop that creates rows and columns for the `bug` items.");
        }
    }
}